...
Publications <- read_excel("Publications.xlsx", 
                           col_types = c("text", "text", "numeric", "numeric", "numeric", "date", "text", 
...
#bind column 9 of Publications dataframe to meds dataframe
meds$price <- Publications$`Pub-Preis per 01.08.2024`
...
meds_sub_num1 <- meds_sub_num1 %>%
  mutate(Last3 = str_extract(Description_Last, "\\d+\\.?\\d*"),   # Extract the last number
...
# how many NAs in overall$unitcost
table(is.na(overall$unit_cost))

#describe the distribution of overall$unit_cost
summary(overall$unit_cost)

# histogram of unit_cost, centered on median
hist(overall$unit_cost, breaks = 0.001, xlim = c(0, 10000), main = "Unit Cost Distribution", xlab = "Unit Cost", col = "lightblue")
...
# change unit_cost into digits instead of scientific notation
